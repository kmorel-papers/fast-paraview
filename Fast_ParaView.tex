...
It is helpful to understand a bit about artistic color theory \cite{Itten} as a base for our discussion. Colormaps translate data, not through the colors themselves, but the \textit{contrast} between the perceivable hues. In discrete colormaps it is the contrast between the individual colors. Scalar colormaps include the interpolation between the color control points which are generated to create a smooth transitions. Discriminatory power is generated by the contrast between the control points as well as the path of interpolation through the colorspace between the control points. 



\fix{Is the use of the word ``hue'' here consistent with how it is used elsewhere in the paper? Throughout the paper, we consider hue as one color property in a hue-saturation-luminance model. The linear maps described above (sometimes) change only saturation and luminance, leaving hue alone. Thus, when this paragraph states that colormaps translate data only through contrast of hues, that would mean that a linear map provides no data because it has no change of hue. Does the paragraph more generally mean perceptual differences in color rather than just differences in hue?}
\\

\fix{Yes, it is confusing. Happy to change the diction. In my community hue refers both to the channel of hue but is also how we speak of individual colors, especially ones that are nuanced and without a specific name. I changed the above to colors and tried to clarify. See what you think.}
...
Artistic Color Contrast Theory, used and demonstrated in paintings since the Renaissance, identifies seven types of color contrast~\cite{Itten}.
The first three are
\begin{inparaenum}[(1)]
...
\fix{I am thoroughly confused by this description. The description in the text does not match the description in the caption. Is this meant to say that the first image shows hue contrast, the second saturation contrast, etc.? If so, how is the left image not also showing, say, cool-warm contrast? Why is the third image (luminance contrast?) two solid colors but the others gradients? 

\\
If the last image is a combination of contrasts, why not show \fast? It is not as clear an example of the types of contrast but I am happy to change it.
\\

And what are the images supposed to demonstrate? Are some showing detail not seen in the others?}
\fix{I rearranged the figure but didn't fix the caption. That is why they are off. I will move the text about specific needs down so that it draws on the Matriz figure, the reason I made that figure. This igure was made to illustrate Hue, saturation and value in a visual context. }


Selecting a colormap best suited to specific tasks, datasets, and data distributions is key to maximizing the capability of color to translate data; however, it is a time-consuming process. Creating custom colormaps such as the one in Figure~\ref{contrast} \fix{one? There are 4. And it was never explained how any were designed for a specific purpose.} requires time and color expertise.  
...
%\fix{Agreed. This does feel repetative.}
...
%\fix{This paragraph also sounds repetative and it feels very awkwardly phrased. I think it will be more clear to just jump into the description of the \blueorange edits.}
